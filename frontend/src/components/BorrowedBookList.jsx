...
const BorrowedBookList = ({ type }) => {
  ...
  const getLoansUrl = () => (type === "history" ? "/loans/history/" : "/loans/my/");

  // Fetch loans on mount / when type changes
  useEffect(() => {
    let isMounted = true;
    ...
    const fetchLoans = async () => {
      setLoading(true);
      try {
        const res = await api.get(getLoansUrl());
        if (isMounted) setLoans(res.data);
      } catch (err) {
  ...
  const handleUpdateLoan = ({ loanId, returned }) => {
    if (returned) {
  ...
  const daysLeft = (due_date) => {
    const diff = Math.ceil(
      (new Date(due_date) - new Date()) / (1000 * 60 * 60 * 24)
    );
    return diff;
  };
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-3 sm:gap-4 md:gap-5 w-full">
        {loans.map((loan) => {
          const coverSrc = loan.book?.cover ? mediaUrl(loan.book.cover) : book_img;
            ...
            >
              ...
              <div className="flex gap-3 sm:gap-4 md:gap-5 items-start flex-1">
                ...
                <div className="rounded-lg sm:rounded-xl overflow-hidden h-24 sm:h-32 md:h-40 w-16 sm:w-24 md:w-32 shrink-0">
                  <img
                    src={coverSrc}
                    alt={loan.book?.title ? `Cover of ${loan.book.title}` : "Book cover"}
                    className="w-full h-full object-cover"
                    loading="lazy"
                    decoding="async"
                    onError={(e) => {
                      e.currentTarget.src = book_img;
                    }}
                  />
                </div>
              ...
              <div className="flex flex-col gap-1 sm:gap-3 text-right shrink-0">
                {type === "current" ? (
                  <>
                    {/* Days Left Indicator */}
                    <div
                      className={`px-2 sm:px-3 md:px-4 py-1 rounded-lg sm:rounded-xl font-semibold text-xs sm:text-sm whitespace-nowrap
                      ${
                  ...
                  <>
                    {/* Return Status */}
                    <div
                      className={`px-2 sm:px-3 md:px-4 py-1 rounded-lg sm:rounded-xl font-semibold text-xs sm:text-sm
                      ${
